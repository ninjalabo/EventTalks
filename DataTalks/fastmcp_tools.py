...
# %% auto 0
__all__ = ['QC_URL', 'ROUTING_SPEC_URL', 'GEOAPIFY_KEY', 'mcp', 'app', 'BASE_DIR', 'STATIC_DIR', 'BASE_URL', 'API_KEY',
           'MAPS_URL', 'Coord', 'client', 'RouteResult', 'route_car', 'route_bike', 'route_walk',
           'route_public_transport', 'generate_chart', 'download_chart', 'generate_image']
...
#| eval: false
# at the top of your camera_talk.py (or wherever you define `app`)

from pathlib import Path
from fastapi.staticfiles import StaticFiles

# 1) Compute the folder *next* to this code file that contains your `static/`
BASE_DIR   = Path(__file__).resolve().parent      # → /home/nata/DataTalks/DataTalks
STATIC_DIR = BASE_DIR / "static"                   # → /home/nata/DataTalks/DataTalks/static

# 2) Mount it under /static
app.mount(
    "/static",
...
def generate_image(
    ...
    """
    Generate images via OpenAI Images API, save image as 'h_bike.jpg'.
    """
    # 1. Ensure your client is configured (or inject API key)
    key = api_key or os.getenv("OPENAI_API_KEY", "")
    if not key:
        raise RuntimeError("OPENAI_API_KEY env-var is missing")

    # 2. Call the API with response_format="b64_json"
    ...
    #image.save("./static/images/h_bike.jpg", format="JPEG", quality=80, optimize=True)
        
    BASE_DIR    = Path(__file__).resolve().parent          # e.g. /home/nata/DataTalks/DataTalks
    STATIC_DIR  = BASE_DIR / "static"                       # -> /home/nata/DataTalks/DataTalks/static
    out_path    = STATIC_DIR / "images" / "h_bike.jpg"
    out_path.parent.mkdir(parents=True, exist_ok=True)     # ensure the images/ folder exists
    image.save(out_path, format="JPEG", quality=80, optimize=True)
    
    logging.info("Saved bike image to %s (exists=%s)", out_path, out_path.exists())
